...
export const Controls = () => {
    ...
    const controlsData = [
        ...
        {img: <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            </svg>, text: 'Change payment method'},
        {img: <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        {img: <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            </svg>, text: 'Cancel recurring'}
    ]

    const controls = controlsData.map((item,i) => {
        const controlsClass = activeItem === i ? 'controls__item active' : 'controls__item';
        return (
            <div key={i} onClick={() => handleActive(i)} className={controlsClass} tabindex="0" aria-label={item.text}>
                {item.img}
                {item.text}
            </div>
        )
